...
class SQLiteDatabaseAPI:
    ...
    du_keys = {
        ...
        "DRB.MeanActiveUeUl": "REAL",
        "DRB.UEThpUl.UEID": "REAL",
        "RRU.PrbUsedUl": "REAL",
        "TB.TotNbrUlInitial": "REAL",
        "TB.TotNbrUlInitial.Qpsk": "REAL",
        "TB.TotNbrUlInitial.16Qam": "REAL",
        "TB.TotNbrUlInitial.64Qam": "REAL"
    }

    end_to_end_delay_keys = {
    "timestamp": "REAL",
    "ueImsiComplete": "INTEGER",
    "delay_ms": "REAL"
    }
    ...
    def __init__(self, simulation_dir, num_ues=None, debug=False):
        ...
        Args:
            ...
            num_ues (int | None): total number of UEs in the simulation
            debug (bool): if True, do not erase the db at the end of the simulation
        """        
        self.simulation_dir = simulation_dir
        self.num_ues = None if num_ues is None else int(num_ues)
        ...
        self._create_table("end_to_end_delay", self.end_to_end_delay_keys)
    ...
    @lock_connection
    def read_rows_at_timestamp(self, table_name: str, timestamp: int):
        query = f"SELECT * FROM {table_name} WHERE timestamp = ?"
        result = self.cursor.execute(query, (timestamp,))
        return result.fetchall()

    @lock_connection
    def read_latest_bsstate_before(self, timestamp: int, cell_ids: list[int] | None = None):
        params = [timestamp]
        cell_filter = ""
        if cell_ids:
            placeholders = ", ".join(["?"] * len(cell_ids))
            cell_filter = f" AND cellid IN ({placeholders})"
        ...
        query = f"""
            ...
            FROM bsState b
            INNER JOIN (
                SELECT cellid, MAX(timestamp) AS max_timestamp
                FROM bsState
                WHERE timestamp <= ?{cell_filter}
                GROUP BY cellid
            ) latest
            ON b.cellid = latest.cellid AND b.timestamp = latest.max_timestamp
        """
        result = self.cursor.execute(query, tuple(params))
        return result.fetchall()
    ...
    def read_end_to_end_delay(self, timestamp: int):
        ...
        query = f"SELECT delay_ms FROM end_to_end_delay WHERE timestamp = ?"
        result = self.cursor.execute(query, (timestamp,)).fetchall()
        return result
