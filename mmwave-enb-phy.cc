...
double
PerfElapsedSeconds(const PerfClock::time_point& start)
{
    return std::chrono::duration<double>(PerfClock::now() - start).count();
}

struct EnbPhyPerfAccumulator
{
    uint64_t calls{0};
    double totalWallSeconds{0.0};
};

EnbPhyPerfAccumulator g_updateSinrPerf;
EnbPhyPerfAccumulator g_startTtiPerf;
} // namespace
    ...
    static TypeId tid =
        TypeId("ns3::MmWaveEnbPhy")
            ...
            .AddAttribute("UpdateSinrEstimatePeriod",
                          ...
                          IntegerValue(25600), // aligned with slower outer-control use cases to reduce redundant SINR refreshes
...
{
    const auto updateSinrStart = PerfClock::now();
    NS_LOG_FUNCTION(this);
    ...
    const double updateSinrWallSeconds = PerfElapsedSeconds(updateSinrStart);
    g_updateSinrPerf.calls++;
    g_updateSinrPerf.totalWallSeconds += updateSinrWallSeconds;
    if (updateSinrWallSeconds > 0.005 || (g_updateSinrPerf.calls % 500) == 0)
    {
        std::cerr << "[PERF] enb_update_sinr call=" << g_updateSinrPerf.calls
                  << " wall_s=" << updateSinrWallSeconds
                  << " avg_wall_ms="
                  << ((g_updateSinrPerf.totalWallSeconds / g_updateSinrPerf.calls) * 1000.0)
                  << " sim_time_s=" << Simulator::Now().GetSeconds()
                  << " cell_id=" << m_cellId
                  << " attached_ues=" << m_ueAttachedImsiMap.size()
                  << " sinr_map_size=" << m_sinrMap.size()
                  << std::endl;
    }
}
...
{
    const auto startTtiStart = PerfClock::now();
    ...
    const double startTtiWallSeconds = PerfElapsedSeconds(startTtiStart);
    g_startTtiPerf.calls++;
    g_startTtiPerf.totalWallSeconds += startTtiWallSeconds;
    if (startTtiWallSeconds > 0.002 || (g_startTtiPerf.calls % 1000) == 0)
    {
        std::cerr << "[PERF] enb_start_tti call=" << g_startTtiPerf.calls
                  << " wall_s=" << startTtiWallSeconds
                  << " avg_wall_ms="
                  << ((g_startTtiPerf.totalWallSeconds / g_startTtiPerf.calls) * 1000.0)
                  << " sim_time_s=" << Simulator::Now().GetSeconds()
                  << " cell_id=" << m_cellId
                  << " slot=" << static_cast<unsigned>(m_slotNum)
                  << " tti_index=" << static_cast<unsigned>(m_ttiIndex)
                  << " curr_slot_ttis=" << static_cast<unsigned>(m_currSlotNumTti)
                  << " attached_devices=" << m_deviceMap.size()
                  << std::endl;
    }
}
